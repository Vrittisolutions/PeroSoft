...
public class DSRGetOtherRcptActivity extends Activity implements OnClickListener{
	Context parent;
	...
	private void initViews() {
		...
		parent = DSRGetOtherRcptActivity.this;
		...
		btnbankdepo.setText("Other Rcpt: "+PetroSoftData.OTHER_RCPT);
		if(PetroSoftData.imgPath!=null){
			Picasso.with(DSRGetOtherRcptActivity.this).load(PetroSoftData.imgPath)/*
						  ...
						  new DialogInterface.OnClickListener() {
							...
							// get user input and set it to result
						    	...
						    	if(Acno!=null&&AcType!=null){
						    		databaseHelper.UpdateOtherRcptTable(TId, Acno, Name, AcType, Amount, Narration);
	...
	public void onClick(View v) {
		...
		case R.id.btnbankdepo:
			...
			PetroSoftData.OTHER_RCPT = splitDatabyspace(data);
			Intent intent = new Intent();
			setResult(PetroSoftData.REQUEST_GET_OTHER_RCPT_DETAILS_FOR_DSR,
					intent);
			finish();
			break;
							...
							.setPositiveButton("Save",
							  new DialogInterface.OnClickListener() {
								...
								// get user input and set it to result
							    	...
							    	getAcCode(Name);
							    	if(Acno!=null&&AcType!=null){
								    databaseHelper.AddOtherRcptTable(Acno, Name, AcType, Amount, Narration);
	...
	private boolean dbvalue() {
		try {
			...
			Cursor cursor = sql.rawQuery("SELECT * FROM OtherRcptTable", null);
	...
	public void updatelist() {
		...
		if (c.getCount()>0){
			...
			btnbankdepo.setText("Other Rcpt:   "+ money);
			c.close();
			db.close();
			db1.close();
		}
