	...
	class GetCustDetails extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			if (PetroSoftData.cust_code == null) {
				...
				Intent ibtnsbmit = new Intent(parent,BillDetails.class);
				ibtnsbmit.putExtra("CallFrom",intentfrom);
				startActivity(ibtnsbmit);
		...
		if (requestCode == 0) {
			...
			if(result != null) {
				...
				//	Toast.makeText(this, "Cancelled", Toast.LENGTH_LONG).show();
					...
					rfidNumberEdittext.setText(scannedvalue.trim());
				}
			} else {
